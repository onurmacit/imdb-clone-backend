...
class APITestCaseExample(APITestCase):
    ...
    def setUp(self):
        ...
        self.movie = Movie.objects.create(title="Test Movie", description="A great movie")
    ...
    def test_add_category_cover_view(self):
        ...
        self.assertEqual(response.status_code, status.HTTP_200_OK)
        self.assertEqual(response.data["detail"], "Category cover uploaded successfully.")
        
        # Test category not found
        invalid_cover_data = cover_data.copy()
        invalid_cover_data["category_id"] = 9999  # Non-existent category
        response = self.client.post(self.category_cover_url, invalid_cover_data, format='json')
